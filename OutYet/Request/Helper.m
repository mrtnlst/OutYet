...
+ (NSArray *)createQueryStringWithArtist:(NSString *)artist withSong:(NSString *)song {
    // Create query string.
    NSMutableString *query = [[NSMutableString alloc]initWithString:artist];
    [query appendString:@"+"];
    [query appendString:song];
    
    // Repace whitespaces by '+' and return array.
    return [[NSArray alloc] initWithObjects:artist, song, [query stringByReplacingOccurrencesOfString:@" " withString:@"+"], nil];
}
